...
# -*- coding: utf-8 -*-
"""
Import BDBRA-style CSV into WholeBIF-RDB (PostgreSQL). v4 Enhanced
Changes from v4:
- Added support for journalscore/methodscore mapping to credibility_rating/summarized_cr
  * journalscore -> credibility_rating
  * methodscore -> summarized_cr
  * citationscore (reserved for future use)

Changes from v3:
- reference_id generation now uses BibTeX to extract author and year
  * Format: "AuthorFamily_YYYY" (e.g., "Zhang_2019")
  * Extracted from BibTeX 'author' and 'year' fields
  * Falls back to v3 logic if BibTeX parsing fails

Previous rules (v3):
- references_tbl.title is taken **from CSV 'reference'** (forced)
- Other rules:
  * doc_link     = https://doi.org/<DOI>
  * bibtex_link  = data:text/plain;... (BibTeX content)
  * doi          = CSV DOI
  * bibtex       = CSV BibTeX
  * journal_names= CSV journal
  * contributor  = "fromBDBRA"
  * connections: dhbasid/dhbarid -> sender/receiver, etc.
- VARCHAR(255) columns are truncated to 255 by default (use --no-truncate to disable).
- CSV decoding: --encoding (default utf-8), --errors (default replace).
"""
...
def extract_author_year_from_bibtex(bibtex: str) -> Tuple[Optional[str], Optional[str]]:
    """
    BibTeX文字列から筆頭著者の姓と年を抽出
    Returns: (author_family_name, year)
    
    Examples:
        @article{Zhang_2019, author = {Li Zhang and John Doe}, year = 2019}
        -> ("Zhang", "2019")
        
        @article{key, author = {Doe, John and Smith, Jane}, year = {2016}}
        -> ("Doe", "2016")
    """
    import re
...
def gen_reference_id(reference_text: str, fallback: str = "", bibtex: str = "") -> str:
    """
    BibTeXから筆頭著者と年を抽出してreference_idを生成
    形式: "AuthorFamily_YYYY" (例: "Zhang_2019")
    
    Args:
        reference_text: 参考文献テキスト（フォールバック用）
        fallback: DOIなどのフォールバック値
        bibtex: BibTeX文字列
    
    Returns:
        reference_id (最大10文字に制限される場合もあり)
    """
    # まずBibTeXから抽出を試みる
    author, year = extract_author_year_from_bibtex(bibtex)
    
    if author and year:
        # 姓_年 の形式で生成
        ref_id = f"{author}_{year}"
        # 長い場合は姓を短縮（10文字以内推奨）
        if len(ref_id) > 20:  # あまり長すぎる場合のみ短縮
            max_author_len = 20 - len(year) - 1  # "_"と年の分を引く
            if max_author_len > 0:
                return f"{author[:max_author_len]}_{year}"
        return ref_id
    
    # BibTeXから取得できない場合は従来のロジック
    ...
    with conn.cursor(cursor_factory=DictCursor) as cur:
        cur.execute(
            ...
            INSERT INTO connections (
                ...
                %(pointers_on_figure)s, %(journal_score)s, %(csi_score)s, %(pder_score)s,
                %(reviewer)s, %(credibility_rating)s, %(summarized_cr)s
            )
...
def build_reference_row(ld: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
    # Force title to come from 'reference'
    ...
    # Build row (title forced from 'reference')
    ref_row = dict(
        ...
        type = typ,
        authors = authors,
        title = reference_text,           # <-- per user's new rule
...
def build_connection_row(ld: Dict[str, Any], reference_id: str) -> Optional[Dict[str, Any]]:
    ...
    # スコアのマッピング:
    #   CSV: journalscore, methodscore, citationscore
    #   RDB: journal_score, pder_score, csi_score
    raw_journal = ld.get("journalscore")
    raw_method  = ld.get("methodscore")
    raw_cite    = ld.get("citationscore")
    ...
    # 数値として解釈
    journal_score = to_float(raw_journal)
    pder_score    = to_float(raw_method)
    csi_value     = to_float(raw_cite)
    ...
    # summarized_cr はとりあえず credibility_rating と同じ
    ...
    con_row = dict(
        ...
        csi_score              = norm(raw_cite) or None,
...
def main():
    ap = argparse.ArgumentParser(description="Import CSV into WholeBIF-RDB (references_tbl + connections). v4 Enhanced - with journalscore/methodscore support")
